...
class LowPassFilter
{
	...
	public void setAlpha(float _alpha)
	{
		...
		{
			StatusLogger.Instance.Error("alpha should be in (0.0., 1.0]");
			return;
		}
		a = _alpha;
	}
...
class OneEuroFilter
{
	...
	void setFrequency(float _f)
	{
		if (_f <= 0.0f)
		{
			StatusLogger.Instance.Error("freq should be > 0");
			return;
		}
		freq = _f;
	}
	...
	void setMinCutoff(float _mc)
	{
		if (_mc <= 0.0f)
		{
			StatusLogger.Instance.Error("mincutoff should be > 0");
			return;
		}
		mincutoff = _mc;
	}
	...
	void setDerivateCutoff(float _dc)
	{
		if (_dc <= 0.0f)
		{
			StatusLogger.Instance.Error("dcutoff should be > 0");
			return;
		}
		dcutoff = _dc;
	}
...
public class OneEuroFilter<T> where T : struct
{
	...
	public OneEuroFilter(float _freq, float _mincutoff = 1.0f, float _beta = 0.0f, float _dcutoff = 1.0f)
	{
		...
		{
			StatusLogger.Instance.Error(type + " is not a supported type");
			return;
		}
	...
	public T Filter<U>(U _value, float timestamp = -1.0f) where U : struct
	{
		...
		if (typeof(U) != type)
		{
			StatusLogger.Instance.Error("WARNING! " + typeof(U) + " when " + type + " is expected!\nReturning previous filtered value");
			currValue = prevValue;
