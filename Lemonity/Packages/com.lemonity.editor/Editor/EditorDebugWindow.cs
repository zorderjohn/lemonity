...
namespace Lemonity.Editor
{
	...
	public class EditorDebugWindow : EditorWindow, IDisposable
	{
		...
		private void OnEnable()
		{
			...
			EditorOptions.OnOptionsChange += RepaintScene;
		...
		public static void DrawStatusLabel()
		{
			...
			GUILayout.Space(2);
			GUILayout.Label(StatusLogger.Instance.Status, EditorStyles.wordWrappedLabel);

			EditorGUI.BeginChangeCheck();
			EditorOptions.Visuals.DebugConsoleLog = EditorGUILayout.Toggle("Print to Console", EditorOptions.Visuals.DebugConsoleLog);
			if (EditorGUI.EndChangeCheck())
			{
				EditorOptions.Dirty = true;
			}
		}
		...
		private void OnSceneGUI(SceneView sceneView)
		{
			...
			if (EditorOptions.Visuals.ShowHandGuides && Options.Mode == WorkingMode.GrabHybrid || Options.Mode == WorkingMode.GrabOneHand || Options.Mode == WorkingMode.GrabTwoHands)
			{
				...
				if (leftDetected && rightDetected && (Options.Mode == WorkingMode.GrabTwoHands || Options.Mode == WorkingMode.GrabHybrid))
				{
					PaintPivot();
				}
			}
		...
		private void PaintHandGuides(HandData hand)
		{
			...
			Handles.DrawWireDisc(v1, cameraRot * Vector3.right, Options.TrackingSpace.PosScale * 0.005f);
			...
			Handles.DrawWireDisc(v1, cameraRot * Vector3.back, Options.TrackingSpace.PosScale * 0.005f);
			Handles.DrawDottedLine(v1, v0, 5);
			...
			Handles.DrawWireDisc(v1, cameraRot * Vector3.down, Options.TrackingSpace.PosScale * 0.005f);
			Handles.DrawDottedLine(v1, v0, 5);

			// Dotted line to the top
			v1 = HandTracking.ToWorldCoordinates(new Vector3(hand.Position.x, y, hand.Position.z));
			Handles.DrawWireDisc(v1, cameraRot * Vector3.up, Options.TrackingSpace.PosScale * 0.005f);
			Handles.DrawDottedLine(v1, v0, 5);
		}
		...
		private void PaintPivot()
		{
			...
			if (!isPinching)
			{
				...
				Handles.color = handleColor;
				Handles.ConeHandleCap(1, centerPos, Quaternion.Euler(90f, 0f, 0f), EditorOptions.Visuals.HandScale * Options.TrackingSpace.PosScale * .03f, EventType.Repaint);
			}
		}
		...
		{
			int numPoints = 37;
			float radius = Options.Heuristic.Radius;
